...
def add_template_repository_to_source_path
  if __FILE__ =~ %r{\Ahttps?://}
    ...
    git :clone => [
      "--quiet",
      "https://github.com/johnmcdowall/modern-rails-template",
      tempdir
...
def assert_foreman
  return if `which foreman` =~ /foreman/
  fail Rails::Generators::Error,
        "This template requires Foreman but it appears" \
...
def run_react_on_rails_installer
  ...
  include_redux = ask_with_default("Install react_on_rails with Redux?", :blue, "no")

  opts = include_redux != "no" ? "--redux" : "--no-redux"
